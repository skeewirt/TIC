// __ZL11scroll_initPh_0 @ 0xD9944 (680 bytes)
...
int __fastcall scroll_init(int result)
{
  ...
  length = v4;
  if ( eLineTotal > 0 && eRefPos >= eLineTotal )
    eRefPos = eLineTotal - 1;
  result = (__int16)v2;
  if ( eRefPos - eRefLine >= v2 || eRefPos < eRefLine )
    eRefLine = eRefPos;
  if ( (__int16)v2 <= eLineTotal )
  {
    if ( (__int16)v2 > eLineTotal - eRefLine )
      eRefLine = eLineTotal - v2;
  }
  else
  {
    eRefLine = 0;
  ...
  if ( eRefLine < 0 )
    eRefLine = 0;
  if ( (__int16)v2 <= eLineTotal && eRefLine > eLineTotal - (__int16)v2 )
    eRefLine = eLineTotal - v2;
  if ( pmes )
  {
    v13 = 16 * v2;
    ...
    v5 = 0;
    v6 = MaxLine;
    for ( i = 2 * eRefLine + mes; ; *(_WORD *)&v9[v8] = *(_WORD *)(i - 2) )
    {
      i += 2;
      if ( v5 >= v6 )
        break;
    ...
    v12 = length;
    v13 = Space;
    result = eMakeTextureMessage(pmes, v9, &v10, 0);
  }
  scroll_type = 0;
  WinFlag = 0;
  return result;
