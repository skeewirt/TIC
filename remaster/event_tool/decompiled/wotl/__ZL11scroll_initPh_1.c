// __ZL11scroll_initPh_1 @ 0xE04D0 (580 bytes)
...
int __fastcall scroll_init(int result)
{
  ...
  if ( jRefPos - jRefLine >= v2 || jRefPos < jRefLine )
    jRefLine = jRefPos;
  if ( (__int16)v2 <= jLineTotal )
  {
    if ( (__int16)v2 > jLineTotal - jRefLine )
      jRefLine = jLineTotal - v2;
  }
  else
  {
    jRefLine = 0;
  }
  if ( jLineTotal <= jRefPos )
    jRefPos = jLineTotal - 1;
  if ( pmes )
  {
    v10 = 256;
    ...
    v5 = 0;
    v6 = MaxLine;
    for ( i = 2 * jRefLine + mes; ; *(_WORD *)&v9[v8] = *(_WORD *)(i - 2) )
    {
      i += 2;
      if ( v5 >= v6 )
        break;
    ...
    v12 = length;
    v13 = Space;
    result = jMakeTextureMessage(pmes, v9, &v10, 0);
  }
  scroll_type = 0;
  WinFlag = 0;
  return result;
